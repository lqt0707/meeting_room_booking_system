...
if docker compose ps | grep -q "Up"; then
    echo -e "${color_green}✅ 前端管理界面服务启动成功！${color_reset}"
    echo -e "${color_green}🌐 访问地址: http://localhost:3001${color_reset}"
else
    echo -e "${color_red}❌ 前端管理界面服务启动失败，请检查日志${color_reset}"
    docker compose logs
    exit 1
fi
...
echo -e "${color_green}🎯 前端管理界面部署完成！${color_reset}"
